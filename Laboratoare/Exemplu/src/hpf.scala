  ...
  def main(args: Array[String]): Unit ={
    ...
    def str1(v:String) : Element[String] = {
      (v) match {
        ...
        case "foarfeca" => Select(0.2 -> h, 0.3 -> p, 0.5->f)
      }
    }

    //strategia celui de al doilea jucator pentru runda a doua
    def str2(v: String) : Element[String] ={
      (v) match {
        case "piatra" => Select(0.3 -> h, 0.4->p, 0.3 ->f)
        case "hartie" => Select(0.2 ->h, 0.6->p, 0.2->f)
        case "foarfeca" => Select(0.3 -> h, 0.3 -> p, 0.4->f)
      }
    }
